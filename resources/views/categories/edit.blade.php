...
@section('title', 'Edit Category - Library Management')

@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-white tracking-tight">Edit Category</h1>
            <p class="text-slate-400 mt-1 text-sm">Update category information.</p>
        </div>
        ...
        <form action="{{ route('categories.update', $category) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ $category->name }}" required class="w-full bg-slate-800/50 border border-slate-700 rounded-xl py-3 px-4 text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-2">Description</label>
                    <textarea name="description" rows="5" class="w-full bg-slate-800/50 border border-slate-700 rounded-xl py-3 px-4 text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">{{ $category->description }}</textarea>
                </div>
            </div>
