...
@section('content')
    ...
    <div class="dark-card rounded-2xl overflow-hidden shadow-sm">
        <div class="p-5 border-b border-slate-700/50 flex justify-between items-center bg-slate-800/30">
            <form action="{{ route('authors.index') }}" method="GET" class="relative w-64">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="w-full bg-[#0f172a] border border-slate-700 rounded-lg py-1.5 pl-9 pr-4 text-sm text-slate-300 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" placeholder="Search authors...">
            </form>
    ...
    <!-- Import Modal -->
    <div id="importAuthorsModal" class="fixed inset-0 z-50 flex items-center justify-center hidden">
        <!-- Backdrop -->
        ...
        <div class="dark-card rounded-2xl w-full max-w-lg p-6 relative z-10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-slate-700">
            ...
            <h3 class="text-xl font-bold text-white mb-2 flex items-center">
                ...
                Import Authors via CSV
            </h3>
            <p class="text-slate-400 text-sm mb-4">Bulk import authors into your catalog. Existing author names will be skipped.</p>
            ...
            <div class="bg-[#0f172a] border border-slate-800 rounded-xl p-4 mb-6">
                ...
                <span class="text-[10px] text-slate-500 block mt-2">Example: "George R.R. Martin, Author of A Song of Ice and Fire series."</span>
            </div>

            <!-- Form -->
            <form action="{{ route('authors.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
    ...
    <script>
        function toggleImportModal() {
            const modal = document.getElementById('importAuthorsModal');
            if (modal) {
