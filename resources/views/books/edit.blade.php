    ...
    <div class="dark-card rounded-3xl p-8 max-w-3xl">
        <form action="{{ route('books.update', $book) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-2">Barcode ID / ISBN (Optional)</label>
                    <input type="text" name="isbn" value="{{ $book->isbn }}" class="w-full bg-slate-800/50 border border-slate-700 rounded-xl py-3 px-4 text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" placeholder="Enter Barcode or ISBN if available">
                </div>
