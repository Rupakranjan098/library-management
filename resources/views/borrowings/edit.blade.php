        ...
        <form action="{{ route('borrowings.update', $borrowing) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="member_name" required placeholder="Enter member name" class="w-full bg-slate-800/50 border border-slate-700 rounded-xl py-3 px-4 text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" value="{{ old('member_name', $borrowing->member->name ?? '') }}">
                </div>
